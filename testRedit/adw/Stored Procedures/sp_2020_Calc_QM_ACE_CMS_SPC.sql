...
-- =============================================
CREATE PROCEDURE [adw].[sp_2020_Calc_QM_ACE_CMS_SPC] 
...
BEGIN TRAN
	...
	--DECLARE @MbrEffectiveDate DATE = '2021-07-01'		
			--Declare Variables
	DECLARE @Metric				Varchar(20)		= 'ACE_CMS_SPC'
	...
	DECLARE @PriorYear		    Varchar(20)		= CONCAT('01/1/',@MeasurementYear - 1)
	...
	JOIN			
					( SELECT		t.SUBSCRIBER_ID
					  FROM			adw.[2020_tvf_Get_ClaimsByValueSet]('ACE_SPC_ENCTR','','','',@MeasurementYear,@PrimSvcDate_End,@CodesetEffDate) t
					  JOIN			adw.[2020_tvf_Get_ClaimsByValueSet]('ACE_SPC_DEN','','','',@MeasurementYear,@PrimSvcDate_End,@CodesetEffDate) s
	...
	FROM			adw.[2020_tvf_Get_ClaimsByValueSet]('ACE_SPC_DEN_EXCLUSIONS','','','',@PriorYear, @PrimSvcDate_End,@CodesetEffDate)a
	UNION
	SELECT DISTINCT Subscriber_ID
	FROM			adw.[2020_tvf_Get_ClaimsByValueSet]('ACE_SPC_DEN_EXCEPTIONS','','','',@PriorYear, @PrimSvcDate_End,@CodesetEffDate)
	...
	FROM			[adw].[2020_tvf_Get_ClaimsByValueSet]('High and Moderate-Intensity Statin Medications','',''
					,'',@MeasurementYear,@PrimSvcDate_End,@CodeSetEffDate) 
...
/***
Usage: 
EXEC [adw].[sp_2020_Calc_QM_ACE_CMS_SPC]	 @ConnectionStringProd	= '[adw].[QM_ResultByMember_History]',
											 @QMDATE				= '2021-07-15',
